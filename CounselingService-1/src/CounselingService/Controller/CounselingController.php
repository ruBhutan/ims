...
class CounselingController extends AbstractActionController
{
	...
	public function __construct(CounselingServiceInterface $counselingService, NotificationServiceInterface $notificationService, AuditTrailServiceInterface $auditTrailService, $serviceLocator)
	{
		...
		*/
        ...
        if($this->usertype == 1){
            $this->userDetails = $this->counselingService->getUserDetails($this->username, $tableName = 'employee_details');
        }
        else if($this->usertype == 2){
            $this->userDetails = $this->counselingService->getUserDetails($this->username, $tableName = 'student');
        ...
        $request = $this->getRequest();
         if ($request->isPost()) {
             $form->setData($request->getPost());
              ...
              $check_counseling_appointment = $this->counselingService->crossCheckCounselingAppointment($subject, $applicant, $applicantType, $counselorId, $appointmentTime, $appointmentDate);

              if($check_counseling_appointment){
              	$message = 'Failure';
              	$this->flashMessenger()->addMessage('You have already seek appointment for the subject '.'"'.$subject.'" '. ' at '.$appointmentTime.' on '.$appointmentDate.' from this particular counselor and it is still pending. Please try for different subject or different time or different date if you want to seek an appointment.');
              }
