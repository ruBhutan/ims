...
class GoodsRequisitionController extends AbstractActionController
{
    ...
    private function sendEmail($userRole,$activity)
    {
         $emailAddress =  $this->goodsRequisitionService->getEmail($userRole, $activity); 
        ...
        $messageBody = "Dear Sir/Madam,<br><h3> Goods requisition has been applied on ".date('Y-m-d').".</h3><br><b>Please click the link below for necessary action.</b><br><u>http://rub-ims.rub.edu.bt/requisition-approval-list</u><p>This is an auto-generated email and Please do not reply because this mail id is not monitored</p>";
           // $this->emailService->sendMailer($emailAddress, $messageTitle, $messageBody);
        if($this->emailService->sendMailer($emailAddress, $messageTitle, $messageBody))
        ...
        if($value == 1){
         ...
         try {
             ...
             // $this->sendGoodsRequisitionEmail($this->employee_details_id, $this->departments_id, $this->departments_units_id, $this->userrole);
              // Send mail
		      if ($this->sendEmail($this->userrole,'Goods Requisition'))
              //
                  $this->flashMessenger()->addMessage('Email Notification has been sent to respective head for the requisition approval');
             else
                  $this->flashMessenger()->addMessage('Email Notification failed!!!! Please inform your respective head for the requition approval physically');
