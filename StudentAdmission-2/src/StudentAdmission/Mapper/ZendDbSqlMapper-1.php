...
class ZendDbSqlMapper implements StudentAdmissionMapperInterface
{
    ...
    **/
    public function getReportedStudentList($stdName, $stdCid, $stdProgramme, $stdId)
    {
        $sql = new Sql($this->dbAdapter);
        $select = $sql->select();
        ...
        $select->from(array('t1' => 'student'))
               ...
               ->where(array('t1.enrollment_year = YEAR(NOW())', 't1.dzongkhag is NULL', 't1.programmes_id' => $stdProgramme))
               ->order(array('t1.student_id ASC'))
               ->where->notLike('t1.student_id', "TEMP_%");                          
    ...
    public function getStudentHouseList($programmesId)
    {
        ...
        $select->from(array('t1' => 'student'))
               ...
               ->where(array('t1.enrollment_year = YEAR(NOW())', 't1.programmes_id' => $programmesId, 't2.semester_id' => '1', 't2.academic_year' => $academic_year))
    ...
    public function getEditHouseStudentList($programmesId, $yearId, $organisation_id)
    {
        ...
        $select->from(array('t1' => 'student_house_details'))
               ...
               ->where(array('t4.academic_year' => $academic_year, 't1.student_house_id is NOT NULL', 't2.programmes_id' => $programmesId, 't4.year_id' => $yearId, 't4.academic_session_id' => $current_academic_session));
              // ->order('id ASC');
    ...
    */
	...
	public function findAllReportedStudent($organisation_id)
	{
        ...
        public function getStudentPermanentAddrDetails($id)
        {
            $sql = new Sql($this->dbAdapter);
            $select = $sql->select();
   ...
   // Function to save new registered student who have reported
    ...
    public function listAllNewStudentFile($tableName, $organisation_id)
    {
	   ...
	   // Function to save new student adding from college
		public function saveNewStudent(AddNewStudent $studentAdmissionObject, $programmes_id, $year_id)
		{
		$studentAdmissionData = $this->hydrator->extract($studentAdmissionObject);
		unset($studentAdmissionData['id']);
    ...
    public function getStudentData($tableName, $id)
    {
        ...
        //if($tableName == 'item_sub_category'){
            $select->from(array('t1' => $tableName));
            $select->columns(array('id', 'first_name', 'middle_name', 'last_name', 'gender', 'date_of_birth', 'cid', 'parent_name', 'parents_contact_no', 'relationship_id', 'admission_year', 'student_type_id', 'programme_id', 'registration_type', 'organisation_id'));
            //$select->where->like('id = ?' => $code);
            $select->where(array('t1.id = ?' => $id));
        //}
    ...
    public function updateEditedStudentSection($data, $programmesId, $yearId, $organisation_id)
    {
        ...
        if($data != NULL)
        {
            ...
            foreach ($data as $value) {
                ...
                $action = new Update('student_semester_registration');
                $action->set($studentAdmissionData);
                $action->where(array('id = ?' => $editedSectionIds[$i]));
    ...
    public function saveNewStudentHouse($data1, $programmesId)
    {
        $i = 1;
        $houseIds = array();
        $houseData = $this->getStudentHouseList($programmesId);
...
/**
	...
	*/
    ...
    public function listAllStudentHouse($tableName)
    {
         $sql = new Sql($this->dbAdapter);
            $select = $sql->select();
    ...
    **/
    public function getStudentLists($stdName, $stdId, $stdCid, $stdProgramme, $organisation_id)
    {
        ...
        $select->from(array('t1' => 'student'))
               ...
               ->join(array('t4' => 'gender'),
                    't4.id = t1.gender', array('stdGender' => 'gender'))
               ->where(array('t1.organisation_id = ?' => $organisation_id, 't1.student_status_type_id' => '1' ))
               ->where->notLike('t1.student_id', "TEMP_%");                          
    ...
    public function getProgrammeChangeStudentLists($stdProgramme, $stdYear, $stdName, $stdId, $organisation_id)
    {
       ...
       // $current_academic_session = $this->getAcademicSession($organisation_id);      
        ...
        $select = $sql->select();
         ...
         $select->from(array('t1' => 'student'))
               ->join(array('t2' => 'student_semester_registration'),
                    't1.id = t2.student_id', array('semester_id', 'student_section_id', 'year_id'))
               ->join(array('t3' => 'programmes'),
                    't3.id = t1.programmes_id', array('programme_name'))
               ->join(array('t4' => 'student_semester'),
                    't4.id = t2.semester_id', array('semester'))
               ...
               ->join(array('t8' => 'programme_year'),
                    't8.id = t2.year_id', array('year'))
               ->where(array('t1.programmes_id' => $stdProgramme, 't2.year_id' => $stdYear))
               ->where->notLike('t1.student_id', "TEMP_%");
    ...
    public function updateStudentChangeProgramme($programme_data, $stdProgramme, $stdYear, $organisation_id, $changeProgramme, $updateDate, $updateBy)
    {
        $i = 1;
        $studentIds = array();
        $studentData = $this->getProgrammeChangedStudentId($stdProgramme, $stdYear, $organisation_id);
        ...
        if($programme_data != NULL)
        {
            ...
            foreach($programme_data as $data){
                $semester = $this->getstdCurrentSemester($studentIds[$i]);
    ...
    {
        $studentAdmissionData['programmes_Id'] = $updated_programme;
