...
class ZendDbSqlMapper implements AlumniMapperInterface
{
    ...
    */
    ...
    public function getAlumniMemberList($alumniProgramme, $alumniBatch, $alumniName, $organisation_id)
    {
        ...
        if($alumniProgramme == 'All' && $alumniBatch == 'All' && $alumniName == NULL){
             $select->from(array('t1' => 'alumni'))
                    ->columns(array('id', 'student_id','first_name', 'middle_name', 'last_name', 'cid', 'date_of_birth', 'graduation_year', 'alumni_status', 'alumni_programmes_id'))
                    ->join(array('t2' => 'alumni_programmes'),
                            't2.id = t1.alumni_programmes_id', array('programme_name'))
   ...
   /* public function getRegisteredMemberList($memProgramme, $memYear, $memName)
	...
	public function getAlumniPermanentAddress($id)
	{
		...
		$select->from(array('t1' => 'alumni'))
			   ->join(array('t2' => 'past_student'),
					't2.cid = t1.cid', array('village', 'gewog', 'dzongkhag'))
				->join(array('t3' => 'village'),
					't3.id = t2.village', array('village_name'))
				->join(array('t4' => 'gewog'),
				't4.id = t2.gewog', array('gewog_name'))
				->join(array('t5' => 'dzongkhag'),
				't5.id = t2.dzongkhag', array('dzongkhag_name'))
			   ->where(array('t1.id' => $id));
	...
	public function getAlumniEmploymentDetails($id)
	{
		...
		$select->from(array('t1' => 'alumni'))
			   ->join(array('t2' => 'study_level'),
					't2.id = t1.qualification_level_id', array('study_level'))
			   ->where(array('t1.id' => $id));
