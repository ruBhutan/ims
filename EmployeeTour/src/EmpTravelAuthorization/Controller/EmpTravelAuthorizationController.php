...
use EmpTravelAuthorization\Form\EmpTravelPaymentForm;
...
class EmpTravelAuthorizationController extends AbstractActionController
{
             ...
             if ($form->isValid()) { 
			    ...
			    $travel_auth_date = $auth_date;
             	...
             	if($travel_auth_date == $check_applied_travel){
					...
					$message = 'Failure';
					$this->flashMessenger()->addMessage("Start Date should not be less than current date");
				 }
    ...
    public function updateEmpTravelDetailStatusAction()
    {
     ...
     public function downloadTravelOrderFileAction()
     {
		...
		$headers->addHeaderLine('Content-Type', $mimetype)
				->addHeaderLine('Content-Disposition: inlie', 'attachment; filename="'.basename($file).'"')
				->addHeaderLine('Content-Length', filesize($file))
     ...
     public function empTravelReportAction() {
	    $this->loginDetails();
	    
	    $organizations = [];
	    
	    $organization = ($this->getRequest()->getPost('organization'))? $this->my_decrypt($this->getRequest()->getPost('organization'), $this->keyphrase): $this->organisation_id;
	    ...
	    foreach ( $result as $res ) {
	      $organizations[$res['id']] = $res['organisation_name'];
	    }
	    
	    $travelList = $this->empTravelAuthorizationService->listAllTravels('all', $organization, $this->userrole, $this->employee_details_id);
	    return array(
	        'keyphrase' => $this->keyphrase,
  ...
  public function makePaymentAction () {
    ...
    if (is_numeric($id)) {
      $form = new EmpTravelPaymentForm();
      $travelPaymentDetailsModel = new TravelPaymentDetails();
      $form->bind($travelPaymentDetailsModel);
      $paymentTypes = $this->empTravelAuthorizationService->fetchPaymentTypes();
      $travelDetails = $this->empTravelAuthorizationService->getTravelDetails($id)->toArray();
      $travelPaymentList = $this->empTravelAuthorizationService->fetchTravelPaymentList('travel_authorization_id', $id);
      ...
      if ($request->isPost()) {
        ...
        if ($form->isValid()) {
          try {
            $travelPaymentDetailsModel->setTravel_Authorization_Id($id);
            $this->empTravelAuthorizationService->saveTravelPaymentDetails($travelPaymentDetailsModel);
            $this->empTravelAuthorizationService->updateTravelAuthorizationStatus($id);
            $this->auditTrailService->saveAuditTrail("INSERT", "Travel Payment Details", "ALL", "SUCCESS");
            $this->flashMessenger()->addMessage(' Travel payment against the request made successfully');
            return $this->redirect()->toUrl('/public/emptravel/makepayment/'.$id_from_route);
          } catch (\Exception $ex) {
            $message = 'Failure';
      ...
      return array(
          ...
          'travelDetails' => $travelDetails[0],
          'travelPaymentList' => $travelPaymentList,
      );
    }
  }
  ...
  public function deletePaymentAction () {
    ...
    if (is_numeric($id)) {
      $travelPayment = $this->empTravelAuthorizationService->fetchTravelPaymentList('id', $id);
      $this->empTravelAuthorizationService->deletePaymentDetails($id);
      $this->empTravelAuthorizationService->updateTravelAuthorizationStatus($travelPayment[0]['travel_authorization_id']);
      $this->auditTrailService->saveAuditTrail("DELETE", "Delete Travel Authorization Details", "ALL", "SUCCESS");
      $this->flashMessenger()->addMessage(' Travel authorization details successfully deleted');
      return $this->redirect()->toUrl('/public/emptravel/makepayment/'.$this->my_encrypt($travelPayment[0]['travel_authorization_id'], $this->keyphrase));
    }
    return $this->redirect()->toRoute('emptravelreport');
  }
  ...
  public function editPaymentAction () {
    ...
    if (is_numeric($id)) {
      ...
      $paymentTypes = $this->empTravelAuthorizationService->fetchPaymentTypes();
      $travelPayment = $this->empTravelAuthorizationService->fetchTravelPaymentList('id', $id);
      $travelDetails = $this->empTravelAuthorizationService->getTravelDetails($travelPayment[0]['travel_authorization_id'])->toArray();
      ...
      if ($request->isPost()) {
        ...
        if ($form->isValid()) {
          try {
            $travelPaymentDetailsModel->setId($id);
            $travelPaymentDetailsModel->setTravel_Authorization_Id($travelPayment[0]['travel_authorization_id']);
            $this->empTravelAuthorizationService->saveTravelPaymentDetails($travelPaymentDetailsModel);
            $this->empTravelAuthorizationService->updateTravelAuthorizationStatus($travelPayment[0]['travel_authorization_id']);
            $this->auditTrailService->saveAuditTrail("UPDATE", "Travel Payment Details", "ALL", "SUCCESS");
            $this->flashMessenger()->addMessage(' Travel payment against the request was updated successfully');
            return $this->redirect()->toUrl('/public/emptravel/makepayment/'.$this->my_encrypt($travelPayment[0]['travel_authorization_id'], $this->keyphrase));
          } catch (\Exception $ex) {
            $message = 'Failure';
            $this->flashMessenger()->addMessage($ex->getMessage());
