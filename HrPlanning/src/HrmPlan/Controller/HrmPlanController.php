...
class HrmPlanController extends AbstractActionController
{
	...
	public function __construct(HrmPlanServiceInterface $hrmPlanService, NotificationServiceInterface $notificationService, AuditTrailServiceInterface $auditTrailService, $serviceLocator)
	{
		...
		*/
		...
		$this->userDetails = $this->hrmPlanService->getUserDetails($this->username, $this->usertype);
        $this->userImage = $this->hrmPlanService->getUserImage($this->username, $this->usertype);
	}
			...
			$submitForm = new SubmitProposalForm();
			 ...
			 if ($request->isPost()) {
				 ...
				 if ($form->isValid()) {
					 ...
					 try {
						 $this->hrmPlanService->save($hrmPlanModel, $data);
						  $this->auditTrailService->saveAuditTrail("EDIT", "HRM Plan", "ALL", "SUCCESS");
			...
			$request = $this->getRequest();
			 if ($request->isPost()) {
				 try {
						 ...
						 $this->auditTrailService->saveAuditTrail("DELETE", "HRM Plan", "ALL", "SUCCESS");
	...
	public function approveHrmProposalAction()
	{
		...
		if(is_numeric($id)){
			try {
				 ...
				 $this->auditTrailService->saveAuditTrail("UPDATE", "HRM Plan", "ALL", "SUCCESS");
