...
class ZendDbSqlMapper implements EmpResignationMapperInterface
{
	...
	*/
	...
	public function getResignationType($resignationType)
	{
		...
		$select->from(array('t1' => 'resignation_type')) 
				->columns(array('resignation_type'))
				->where(array('t1.id' => $resignationType));
	...
	public function getSupervisorEmailId($userrole, $departments_units_id)
	{
		...
		$select->where(array('t1.role' =>$userrole, 't1.role_department' => $departments_units_id, 't1.type' => 'Staff Resignation'));
	...
	public function getRecordedResignedEmpDetails($employee_details_id, $organisation_id)
	{
		...
		if(!empty($emp_id)){
			...
			$select1->from(array('t1' => 'employee_details'))
					->columns(array('first_name', 'middle_name', 'last_name', 'emp_id', 'departments_units_id', 'departments_id'))
					->join(array('t2' => 'users'),
						't2.username = t1.emp_id', array('role'))
					->where(array('t2.username' => $emp_id, 't2.region' => $organisation_id));
	...
	{ 
		...
		$select->from(array('t1' => 'user_workflow'))
			   ->columns(array('auth', 'department', 'organisation'));
		$select->where(array('t1.role' =>$staff_role, 't1.role_department' => $departments_units));
		$select->where->like('t1.type', '%Dues');
		...
		foreach($resultSet as $set){
			$authorizee_details[$set['auth']][$set['department']][$set['organisation']] = $set['organisation'];
        }
        
        if(!empty($authorizee_details)){
        	foreach($authorizee_details as $auth => $value){ 
	        	foreach($value as $department => $value1){
	        		foreach($value1 as $organisation){
	        			$sql1 = new Sql($this->dbAdapter);
						$select1 = $sql1->select();
