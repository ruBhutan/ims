...
use Hostel\Form\AllocateHostelRoomForm;
...
class HostelController extends AbstractActionController
{
	...
	public function addHostelDetailAction()
    {
		...
		$hostelType = $this->hostelService->listSelectData($tableName = 'student_category', $columnName = 'student_category', NULL);
	...
	public function allocateHostelAction()
    {
		...
		$programmeYear = $this->hostelService->getStudentNoByYear($this->organisation_id);// var_dump($programmeYearStudent); echo '<br>';
		/*foreach($programmeYearStudent as $key => $value){
			$programmeYear = $value;
		}//var_dump($programmeYear); die();*/
		$message = NULL;
	...
	public function allocateHostelRoomAction()
	{
		$this->loginDetails();
		$id_from_route = $this->params()->fromRoute('id', 0);
		$id = $this->my_decrypt($id_from_route, $this->keyphrase);
        ...
        if(is_numeric($id)){
			$form = new AllocateHostelRoomForm();
			$hostelModel = new AllocateHostelRoom();
			$form->bind($hostelModel);

			$hostelDetails = $this->hostelService->findHostel($id);
			$hostelRoom = $this->hostelService->getUnallocatedHostelRoom($id, $this->organisation_id, 'Hostel Room');
			$hostelStudent = $this->hostelService->getHostelUnallocatedStudent($id, $this->organisation_id, 'Student');
			...
			if($request->isPost()){
				...
				if($form->isValid()){ 
					try{
						$this->hostelService->allocateHostelRoom($hostelModel);
						$this->notificationService->saveNotification('Hostel Allocation', NULL, NULL, 'Hostel Room Allocation');
						$this->auditTrailService->saveAuditTrail("INSERT", "Student Hostels", "ALL", "SUCCESS");
						$this->flashMessenger()->addMessage('Hostel Room was successfully allocation to student');
						return $this->redirect()->toRoute('allocatedhosteldetails', array('id' => $this->my_encrypt($id, $this->keyphrase)));
					}
					catch(\Exception $e){
						die($e->getMessage());
					}
	...
	public function removeHostelAllocatedStudentAction()
	{
		$this->loginDetails();
		...
		$hostel_id = $this->hostelService->getHostelId($id); 

		if(is_numeric($id)){
			$result = $this->hostelService->removeHostelAllocatedStudent($id);
			$this->auditTrailService->saveAuditTrail("DELETE", "Student Hostel", "ALL", "SUCCESS");
			$this->flashMessenger()->addMessage("You have successfully remove student from allocated hostel room");
			return $this->redirect()->toRoute('allocatedhosteldetails', array('id' => $this->my_encrypt($hostel_id, $this->keyphrase)));
		}else{
			return $this->redirect()->toRoute('allocatedhosteldetails', array('id' => $this->my_encrypt($hostel_id, $this->keyphrase)));
		}
		...
		$request = $this->getRequest();
         if ($request->isPost()) {
             ...
             if ($form->isValid()) {
				...
				$hostelDetail = $this->hostelService->getSelectedHostelDetails($hostelName);
             }
         }
		 else {
	...
	public function addAdditionalHostelRoomAction()
	{
		$this->loginDetails();
        ...
        if(is_numeric($id)){
	        ...
	        $request = $this->getRequest();
	         if ($request->isPost()) {
	             ...
	             if ($form->isValid()) { 
	                 try {
						 $this->hostelService->saveAdditionalRoom($hostelModel);
						 $this->auditTrailService->saveAuditTrail("UPDATE", "Hostel Lists", "ALL", "SUCCESS");
						 $this->flashMessenger()->addMessage('Additional number of room was successfully added');
	...
	// Function to delete the hostel room
	public function deleteAddedHostelRoomAction()
	{
		$this->loginDetails();
        ...
        if(is_numeric($id)){ 
			$check_room_assigned = $this->hostelService->crossCheckAssignedHostelRoom($id);
			if(!empty($check_room_assigned)){
				$message = 'Failure';
				$this->flashMessenger()->addMessage('Unable to delete this particular room since you have already allocated to student. Before you delete this room please remove/ delete allocated student from this particular room');
				
			}else{
				try {
					$result = $this->hostelService->deleteAddededHostelRoom($id);
					$this->auditTrailService->saveAuditTrail("DELETE", "Hostel Room", "ALL", "SUCCESS");
					$this->flashMessenger()->addMessage("You have successfully deleted the hostel room");
					return $this->redirect()->toRoute('viewhostelroom');
				}
				catch(\Exception $e) {
						die($e->getMessage());
	...
	public function studentSelfHostelAction()
    {
    	$this->loginDetails();
    	$username = $this->username;
		$selfhostelsdetail = $this->hostelService->selfHostelsDetail($this->student_id, $this->organisation_id);
		
		//var_dump($selfhostelsdetail); die;
        return array(
        	'selfhostelsdetail' => $selfhostelsdetail,
        );            
    }
