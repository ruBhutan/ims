...
class ZendDbSqlMapper implements EmployeeLeaveMapperInterface
{
	...
	*/
	...
	{ 
		...
		foreach($resultSet as $tmp_data){
			$type_authorisation[$tmp_data['type']][$tmp_data['role']][$tmp_data['role_department']] = $tmp_data['role_department'];
		} 
		...
		/*if($officiating_role){
			...
			foreach($resultSet3 as $tmp_data3){
				$type_authorisation[$tmp_data3['type']][$tmp_data3['role']][$tmp_data3['role_department']] = $tmp_data3['role_department'];
			} 
		}*/
		...
		if(!empty($type_authorisation)){
			foreach($type_authorisation as $type => $value){	
				...
				foreach($value as $role => $value2){
					$authorizee_emp_ids = $this->getEmployeeIdByRoles($role); 
					
					foreach($value2 as $role_department){ 
	...
	*/
	...
	private function checkAuthorizedRole($userrole, $organisation_id)
	{
		...
		$role = array();
		foreach($resultSet as $set){
            $role[$set['type']][$set['role']][$set['role_department']] = $set['role'];
        }
        return $role;
	...
	*/
	...
	private function getEmployeeIdByRoles($role)
	{
		$sql = new Sql($this->dbAdapter);
		$select = $sql->select();
