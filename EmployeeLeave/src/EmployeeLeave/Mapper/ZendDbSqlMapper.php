...
class ZendDbSqlMapper implements EmployeeLeaveMapperInterface
{
	...
	*/
	...
	public function getSupervisorIdByRole($supervisor_role, $supervisor_dept)
	{
		...
		$select->from(array('t1' => 'employee_details'))
			->join(
				array('t2' => 'users'),
				't1.emp_id = t2.username',
				array('username', 'role', 'user_status_id')
			);
	...
	public function getSupervisorEmail($supervisor_role, $supervisor_dept, $supervisor_org)
	{
		...
		$select->from(array('t1' => 'users'))
			->join(
				array('t2' => 'employee_details'),
				't2.emp_id = t1.username',
				array('email')
			);
	...
	public function getApprovedLeaveApplicantDetails($id)
	{
		...
		$select->from(array('t1' => 'emp_leave'))
			->join(
				array('t2' => 'employee_details'),
				't2.id = t1.employee_details_id',
				array('first_name', 'middle_name', 'last_name', 'email')
			)
			->join(
				array('t3' => 'employee_details'),
				't3.id = t1.approved_by',
				array('afirst_name' => 'first_name', 'amiddle_name' => 'middle_name', 'alast_name' => 'last_name')
			);
	...
	public function getApprovedLeaveSubstitution($id)
	{
		...
		$select->from(array('t1' => 'emp_leave'))
			->join(
				array('t2' => 'employee_details'),
				't2.id = t1.substitution',
				array('sub_first_name' => 'first_name', 'sub_middle_name' => 'middle_name', 'sub_last_name' => 'last_name', 'sub_email' => 'email')
			);
		$select->where(array('t1.id' => $id));
	...
	*/
	...
	public function findLeave($id)
	{
		...
		$select->from(array('t1' => 'emp_leave'))
			->join(
				array('t2' => 'employee_details'),
				't1.employee_details_id = t2.id',
				array('first_name', 'middle_name', 'last_name', 'emp_id')
			)
			->join(
				array('t3' => 'employee_details'),
				't1.substitution = t3.id',
				array('sub_first_name' => 'first_name', 'sub_middle_name' => 'middle_name', 'sub_last_name' => 'last_name', 'sub_emp_id' => 'emp_id')
			)
	...
	public function getEmployeeOccupationalGroup($employee_details_id)
	{
		...
		$select->from(array('t1' => 'emp_position_title')) // base table
			->join(
				array('t2' => 'position_title'), // join table with alias
				't1.position_title_id = t2.id',
				array('position_category_id')
			) // join expression
			->join(
				array('t3' => 'position_category'),
				't3.id = t2.position_category_id',
				array('major_occupational_group_id')
			)
			->join(
				array('t4' => 'major_occupational_group'),
				't4.id = t3.major_occupational_group_id',
				array('id', 'major_occupational_group')
			)
	...
	*/
	...
	public function getDepartmentUnits($organisation_id)
	{
		...
		$select->from(array('t1' => 'departments'))
			...
			->join(
				array('t2' => 'department_units'),
				't1.id = t2.departments_id',
				array('id')
			);
	...
	public function getEmpApprovedLeaveList($organisation_id)
	{
		...
		$select->from(array('t1' => 'emp_leave'))
			->join(
				array('t2' => 'emp_leave_category'),
				't1.emp_leave_category_id = t2.id',
				array('leave_category')
			)
			->join(
				array('t3' => 'employee_details'),
				't1.employee_details_id = t3.id',
				array('first_name', 'middle_name', 'last_name', 'emp_id')
			);
	...
	*/
	...
	public function updateLeaveBalance($id)
	{
		...
		$annual_leave_balance = $employee_leave_balance['annual_leave'];
		...
		if ($leave_type == 'Casual Leave') {
			...
			$leave_balance = $annual_leave_balance - $days_of_leave;
			$action = new Update('emp_leave_balance');
			$action->set(array('annual_leave' => $leave_balance));
			$action->where(array('employee_details_id = ?' => $employee_details_id));
		} else {
			return;
		}
	...
	public function updateEmpApprovedLeave(CancelledLeave $leaveObject)
	{
		...
		$casual_leave_balance = $employee_leave_balance['casual_leave'];
		$earned_leave_balance = $employee_leave_balance['earned_leave'];
		$annual_leave_balance = $annual_leave_balance['annual_leave'];
		...
		if ($leave_category == 'Casual Leave') {
			...
			$leave_balance = $annual_leave_balance + $leaveData['no_Of_Days'];
			$leave_action = new Update('emp_leave_balance');
			$leave_action->set(array('annual_leave' => $leave_balance));
			$leave_action->where(array('employee_details_id = ?' => $leaveData['employee_Details_Id']));
		} else {
			return;
		}
	...
	public function getAppliedLeaveCategory($emp_leave_id)
	{
		...
		$select->from(array('t1' => 'emp_leave'))
			->join(
				array('t2' => 'emp_leave_category'),
				't2.id = t1.emp_leave_category_id',
				array('leave_category')
			)
	...
	*/
	...
	public function getEmployeeLeaveBalance($employee_details_id)
	{
		...
		$select->from(array('t1' => 'emp_leave_balance'))
			->columns(array('casual_leave', 'earned_leave', 'annual_leave'))
		...
		foreach ($resultSet as $set) {
			...
			$leave_details['annual_leave'] = $set['annual_leave'];
		}
	...
	*/
	...
	public function getOfficiatingList($employee_details_id)
	{
		...
		$select->from(array('t1' => 'employee_details'))
			...
			->join(
				array('t2' => 'user_workflow_officiating'),
	...
	public function getEmployeeList($organisation_id)
	{
		...
		$select->from(array('t1' => 'employee_details'))
			...
			->join(
				array('t2' => 'emp_position_level'),
				't1.id = t2.employee_details_id',
				array('position_level_id')
			)
			->join(
				array('t3' => 'position_level'),
				't2.position_level_id = t3.id',
				array('position_level')
			)
	...
	*/
	...
	public function getLeaveTaken($employee_details_id, $type)
	{
		...
		if ($type == "Extra-Ordinary Leave (EOL)") {
			...
			$leave_id = $this->getLeaveId($type);
			$select->from(array('t1' => 'emp_leave_balance'))
				->columns(array('employee_details_id', 'annual_leave'))
				->where(array('employee_details_id' => $employee_details_id));
	...
	public function getLeaveBalance($employee_details_id)
	{
		...
		$select->from(array('t1' => 'emp_leave_balance'))
			->columns(array('earned_leave', 'casual_leave', 'annual_leave'))
			->where(array('t1.employee_details_id' => $employee_details_id));
		...
		foreach ($resultSet as $set) {
			...
			$leave_balance['annual_leave'] = $set['annual_leave'];
		}
		...
		if ($organisation_id != 1) {
			$select->from(array('t1' => 'emp_leave_balance'))
				->join(
					array('t2' => 'employee_details'),
					't2.id = t1.employee_details_id',
					array('first_name', 'middle_name', 'last_name', 'emp_id', 'organisation_id')
				)
				->join(
					array('t3' => 'emp_position_title'),
					't2.id = t3.employee_details_id',
					array('employee_details_id')
				)
				->join(
					array('t4' => 'position_title'),
					't4.id = t3.position_title_id',
					array('position_title')
				)
				->join(
					array('t5' => 'employee_details'),
					't1.update_by_employee_id = t5.id',
					array('sub_first_name' => 'first_name', 'sub_middle_name' => 'middle_name', 'sub_last_name' => 'last_name')
				);
			...
			$select->from(array('t1' => 'emp_leave_balance'))
				...
				->join(
					array('t3' => 'organisation'),
					't3.id = t2.organisation_id',
					array('organisation_name')
				)
				->join(
					array('t4' => 'emp_position_title'),
					't2.id = t4.employee_details_id',
					array('employee_details_id')
				)
				->join(
					array('t5' => 'position_title'),
					't5.id = t4.position_title_id',
					array('position_title')
				)
				->join(
					array('t6' => 'employee_details'),
					't1.update_by_employee_id = t6.id',
					array('sub_first_name' => 'first_name', 'sub_middle_name' => 'middle_name', 'sub_last_name' => 'last_name')
				);
