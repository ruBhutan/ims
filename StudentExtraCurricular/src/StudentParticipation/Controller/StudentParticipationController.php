...
class StudentParticipationController extends AbstractActionController
{
         ...
         if ($request->isPost()) {
             ...
             if ($form->isValid()) {
                 try {
					 ...
					 $this->flashMessenger()->addMessage('Participation Category was successfully added');
					  return $this->redirect()->toRoute('stdparticipationcategory');
	...
	public function editStudentParticipationCategoryAction()
    {
        ...
        if(is_numeric($id)){ 
			...
			$categoryDetail = $this->participationService->getStudentParticipationCategoryDetails($id);
	        $message = NULL;
	        $request = $this->getRequest();
	         if ($request->isPost()) {
	             $form->setData($request->getPost());
	             if ($form->isValid()) {
	                 try {
						 ...
						 $this->auditTrailService->saveAuditTrail("EDIT", "Student Participation Category", "ALL", "SUCCESS");
						  $this->flashMessenger()->addMessage('Participation Category was successfully edited');
					 	return $this->redirect()->toRoute('stdparticipationcategory');
					 }
					 catch(\Exception $e) {
							 die($e->getMessage());
        	...
        	$this->redirect()->toRoute('stdparticipationcategory');
        }
    } 
	...
	public function addStdParticipationAction()
    {
        ...
        if(is_numeric($id)){
			...
			$participation_category = $this->participationService->listSelectData($tableName = 'student_participation_category', $columnName = 'participation_type', $this->organisation_id);
