...
class ZendDbSqlMapper implements EmpTransferMapperInterface
{
	...
	*/
	...
	public function getTransferOrganisationList($id)
	{
		$sql = new Sql($this->dbAdapter);
		$select = $sql->select();
		...
		$select->from(array('t1' => 'emp_transfer_application'))
				->join(array('t2' => 'organisation'), 
					't1.transfer_request_to = t2.id', array('organisation_name'))
	...
	*/
	...
	public function getAuthorizeedEmailId()
	{   
		$type = "Approved Transfer";
		...
		$select->from(array('t1' => 'user_workflow'))
			   ->join(array('t2' => 'users'),
			   		't2.role = t1.auth', array('username'))
			   ->join(array('t3' => 'employee_details'),
					't2.username = t3.emp_id', array('email'))
		       ->where(array('t1.type'=> $type));
