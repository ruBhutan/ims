...
class ZendDbSqlMapper implements ExaminationsMapperInterface
{
	...
	*/
	...
	public function consolidateMarks($organisation_id)
	{
		...
		$programme = $this->listSelectData('programmes','programme_name', $organisation_id);
		foreach($programme as $programme_id => $programme_name){
			$programme_duration = $this->getProgrammeDuration($programme_id);
			for($programme_year=1; $programme_year<=$programme_duration; $programme_year++){
				$modules = $this->getModuleAllocationByProgramme($programme_id, $organisation_id, $programme_year);
				...
				foreach($modules as $module_allocation_id => $module_code){
					...
					$backyear_students_module_cleared = array_diff_key($backyear_students_list, $backpaper_students_in_module);
					
					foreach($backyear_students_module_cleared as $key => $value){
						unset($student_list[$key]);
					}
					
					//calculate marks for students
					foreach($student_list as $id => $student_id){
						$this->calculateStudentConsolidatedMarks($module_allocation_id, $id, $student_id);
					}
					...
					foreach($backpaper_students as $backpaper_id => $backpaper_student_id){
						$this->calculateStudentConsolidatedMarks($module_allocation_id, $backpaper_id, $backpaper_student_id);
					}
	...
	*/
	...
	private function calculateStudentConsolidatedMarks($academic_module_allocation_id, $id, $student_id)
	{
		...
		$ca_result_components = $this->getConsolidateMarkComponents($academic_module_allocation_id, 'Continuous Assessment');
		$se_result_components = $this->getConsolidateMarkComponents($academic_module_allocation_id, 'Semester Exams');
		$marks = NULL;
	...
	*/
	...
	private function searchBackpaperStudents($module_allocation_id)
	{
		...
		//Refer to database table for complete details
		$ca_assessment_components = $this->getConsolidateMarkComponents($module_allocation_id, 'Continuous Assessment');
		$se_assessment_components = $this->getConsolidateMarkComponents($module_allocation_id, 'Semester Exams');
	...
	private function calculateStudentBackpapers($assessment_components, $assessment_type)
	{
		...
		if($assessment_type != 'Combined'){		
			...
			$select =  $this->db->select
                            ->columns(array('student_id','marks'=>new \Zend\Db\Sql\Expression('SUM(marks)')))
                            ->from('student_consolidated_marks')
                            ->group('student_id');*/
	...
	*/
	...
	public function getSemester($organisation_id)
	{
		...
		foreach($resultSet as $set){
			if($set['academic_event'] == 'Start of Odd Semester'){
				$semester = 'Odd';
			}
			else if($set['academic_event'] == 'Start of Even Semester'){
				$semester = 'Even';
			}
		}
		return $semester;
