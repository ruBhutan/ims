...
class ZendDbSqlMapper implements MedicalRecordMapperInterface
{
	...
	*/
	...
	public function saveDetails(MedicalRecord $recordObject)
	{
		...
		$recordData['from_Date'] = date("Y-m-d", strtotime(substr($recordData['from_Date'],0,10)));
		$recordData['to_Date'] = date("Y-m-d", strtotime(substr($recordData['to_Date'],0,10)));

		$medical_proof = $recordData['medical_Proof'];
		$recordData['medical_Proof'] = $medical_proof['tmp_name'];

		if($recordObject->getId()) { 
			//ID present, so it is an update
			//If the uploaded file is NULL then check for previously uploaded file
			if($recordData['medical_Proof'] == NULL){
				$recordData['medical_Proof'] = $this->getUploadedMedicalFile($recordObject->getId()); 
			}else{
				$recordData['medical_Proof'] =$recordData['medical_Proof'];
			}
	...
	public function getUploadedMedicalFile($id)
	{
		$sql = new Sql($this->dbAdapter);
		$select = $sql->select();
		...
		$uploaded_file = NULL;
		foreach($resultSet as $set){
			$uploaded_file = $set['medical_proof'];
		}
	...
	*/
	...
	public function getMedicalRecordedDetails($id)
	{
		...
		$select->from(array('t1' => 'student_medical_records'))
				->join(array('t2' => 'student'), 
					't1.student_id = t2.id', array('first_name','middle_name','last_name','std_enrol_id'=>'student_id','enrollment_year'))
				->join(array('t3' => 'programmes'), 
					't2.programmes_id = t3.id', array('programme_name'));
		$select->where(array('t1.id' =>$id));
