...
use StudentAdmission\Form\MakeFeesPaymentForm;
use StudentAdmission\Form\AddStudentFeesForm;
use StudentAdmission\Form\SelfFinancedStudentSearchForm;
use StudentAdmission\Form\AddBulkFeeForm;
...
/* Add Masters/Exchange Programme/New Student  like King or Queen Scholarship*/
   ...
   public function editStudentPreviousSchoolDetailsAction()
   {
    ...
    /*Display self financed student details  */
    public function selfFinancedStudentListsAction()
    {
        $this->loginDetails();
        ...
        $form = new SelfFinancedStudentSearchForm();
        ...
        if($request->isPost()){
            ...
            //if($form->isValid()){
                ...
                if ($this->userrole === $this->check_role) {
                  $studentList = $this->studentAdmissionService->getSelfFinancedStudentListsToAdmin($stdName, $stdId,$organisation_id);
                } else {
                  $studentList = $this->studentAdmissionService->getSelfFinancedStudentLists($stdName, $stdId, $this->organisation_id);
                }
            //}
        }
    ...
    public function addStudentFeesAction()
    {
      $this->loginDetails();
      ...
      if (is_numeric($id)) {
        $form = new AddStudentFeesForm();
        $studentFeeDetailsModel = new StudentFeeDetails();
        $form->bind($studentFeeDetailsModel);

        $studentDetails = $this->studentAdmissionService->getStdPersonalDetails($id);
        $studentSemesterList = $this->studentAdmissionService->fetchStudentSemesterList();
        $studentFeeList = $this->studentAdmissionService->listStudentFeeList($id);
        $feeCategories = $this->studentAdmissionService->fetchFeeCategories($this->organisation_id);
        $message = NULL;
        
        for($i=date('Y'); $i<=(date('Y')+5); $i++) {
            $academicYear[($i)."-".($i+1)] = ($i)."-".($i+1);
        }
        ...
        if ($request->isPost()) {
          ...
          if($form->isValid()) {
            try {
              ...
              $isExists = $this->studentAdmissionService->isStudentFeesPaid($data['addstudentfeedetails']);
              if (!$isExists) { // If student fees is not exists then only add in database
                $studentSemesterData = $this->studentAdmissionService->fetchStudentSemesterList($data['addstudentfeedetails']['semester_id']);
                $studentFeeDetailsModel->setYear_Id($studentSemesterData['programme_year_id']); // Set the year_id based on the selected semester
                $this->studentAdmissionService->saveStudentFeeDetails($studentFeeDetailsModel);
                $this->auditTrailService->saveAuditTrail("INSERT", "Add Student Fees", "ALL", "SUCCESS");
                $this->flashMessenger()->addMessage(' Student Fees successfully added');
                return $this->redirect()->toRoute('addstudentfees', array('id' => $id_from_route));
              }
              $message = 'Failure';
              $this->flashMessenger()->addMessage(' Student Fees already exists');
            } catch(\Exception $e){
              $message = 'Failure';
              $this->flashMessenger()->addMessage($e->getMessage());
              return $this->redirect()->toRoute('addstudentfees', array('id' => $id_from_route));
        ...
        return array(
          ...
          'academicYear' => $academicYear,
          'studentSemesterList' => $studentSemesterList,
          'studentFeeList' => $studentFeeList,
          'organisation_id' => $this->organisation_id,
          'student_id' => $id,
          'keyphrase' => $this->keyphrase,
          'message' => $message,
      ...
      return $this->redirect()->toRoute('selffinancedstudentlists');
    }

    public function ajaxfetchfeedetailsAction()
    {
        $this->loginDetails();
        $student_id = $this->getRequest()->getPost('student_id');
        $fee_category = $this->getRequest()->getPost('fee_category');
        $financial_year = $this->getRequest()->getPost('financial_year');

        if (is_numeric($student_id) && is_numeric($fee_category)) {
            $data = $student_programmes_id = [];
            $dbAdapter = $this->serviceLocator->get('Zend\Db\Adapter\Adapter');
            
            $sql = "SELECT programmes_id FROM student WHERE id=". intval($student_id) . " LIMIT 1";
            $results = $dbAdapter->query($sql)->execute();
            if (count($results)) {
                foreach ($results as $row) {
                    $student_programmes_id[] = $row['programmes_id'];
                }
                $sql = "SELECT id as student_fee_structure_id, total_fee FROM student_fee_structure where student_fee_category_id=".$fee_category." AND programmes_id=".$student_programmes_id[0]." AND organisation_id=".$this->organisation_id." AND financial_year='".$financial_year."'";
                $results = $dbAdapter->query($sql)->execute();
                if (count($results)) {
                    foreach ($results as $row) {
                        $data[] = $row;
                    }
                    return new JsonModel(['status' => 'success', 'data' => $data[0]]);
                }
                return new JsonModel(['status' => 'error', 'message' => 'No suitable fee structure found based on fee category and financial year']);
            }
            return new JsonModel(['status' => 'error', 'message' => 'Student not found']);
        }
        return new JsonModel(['status' => 'error', 'message' => 'Invalid student id or fee category']);
    }
    
    public function viewfeesAction()
    {
      $this->loginDetails();
      ...
      if (is_numeric($id)) {
        ...
        return array(
          'studentDetails' => $studentDetails,
          'studentFeeList' => $studentFeeList->toArray(),
          'keyphrase' => $this->keyphrase
        );
      }
      return $this->redirect()->toRoute('selffinancedstudentlists');
    ...
    public function editFeesAction()
    {
      ...
      if (is_numeric($id)) {
        ...
        $feeDetails = $this->studentAdmissionService->fetchStudentFeeDetails($id);
        $studentDetails = $this->studentAdmissionService->getStdPersonalDetails($feeDetails['student_id']);
        $studentSemesterList = $this->studentAdmissionService->fetchStudentSemesterList();
        $feeCategories = $this->studentAdmissionService->fetchFeeCategories($this->organisation_id);
        $message = NULL;
        ...
        if ($request->isPost()) {
          ...
          if($form->isValid()) {
            try {
              $data = $request->getPost()->toArray();
              $isExists = $this->studentAdmissionService->isStudentFeesPaid($data['addstudentfeedetails'], $id); // $id is used to check the unique except the current ID that is used for update operation
              if (!$isExists) { // If student fees is not exists then only add in database
                $studentSemesterData = $this->studentAdmissionService->fetchStudentSemesterList($data['addstudentfeedetails']['semester_id']);
                $studentFeeDetailsModel->setId($id);
                $studentFeeDetailsModel->setYear_Id($studentSemesterData['programme_year_id']); // Set the year_id based on the selected semester
                $this->studentAdmissionService->saveStudentFeeDetails($studentFeeDetailsModel);
                $this->auditTrailService->saveAuditTrail("UPDATE", "Update Student Fees Details", "ALL", "SUCCESS");
                $this->flashMessenger()->addMessage(' Student Fees successfully updated');
                return $this->redirect()->toRoute('addstudentfees', array('id' => $this->my_encrypt($feeDetails['student_id'], $this->keyphrase)));
              }
              $message = 'Failure';
              $this->flashMessenger()->addMessage(' Student Fees already exists');
            } catch(\Exception $e){
    ...
    public function deleteFeesAction()
    {
      $this->loginDetails();
      ...
      if (is_numeric($id)) {
        $feeDetails = $this->studentAdmissionService->fetchStudentFeeDetails($id);
        $isDeleted = $this->studentAdmissionService->deleteStudentFeeDetails($id);
        $this->auditTrailService->saveAuditTrail("DELETE", "Delete Student Fees Details", "ALL", "SUCCESS");
        $this->flashMessenger()->addMessage(' Student Fees Details successfully deleted');
        return $this->redirect()->toRoute('addstudentfees', array('id' => $this->my_encrypt($feeDetails['student_id'], $this->keyphrase)));
      }
      return $this->redirect()->toRoute('selffinancedstudentlists');
    }
    ...
    public function editpaymentAction()
    {
      ...
      if (is_numeric($id)) {
        $form = new MakeFeesPaymentForm();
        $studentFeePaymentDetailsModel = new StudentFeePaymentDetails();
        $form->bind($studentFeePaymentDetailsModel);

        $feePaymentDetails = $this->studentAdmissionService->fetchStudentFeePaymentDetails('id', $id);
        $feeDetailsList = $this->studentAdmissionService->fetchStudentFeePaymentDetails('student_fees_details_id', $feePaymentDetails[0]['student_fees_details_id']);
        $feeDetails = $this->studentAdmissionService->fetchStudentFeeDetails($feePaymentDetails[0]['student_fees_details_id']);
        $studentDetails = $this->studentAdmissionService->getStdPersonalDetails($feePaymentDetails[0]['student_id']);
        $paymentTypes = $this->studentAdmissionService->fetchPaymentTypes();
        $message = NULL;
        ...
        if ($request->isPost()) {
          ...
          if($form->isValid()) {
            try {
              $studentFeeDetails = $this->studentAdmissionService->listStudentFeeList($feeDetails['student_id'], $feeDetails['id'])->toArray();
              if (isset($studentFeeDetails[0])) { // If student fee details found then get the pending amount
                $totalAmount = array_sum(array_map(function($item) use ($id) { return ($item['id']!=$id)? $item['amount']: 0; }, $feeDetailsList));
                $pendingAmount = floatval($feeDetails['amount']) - floatval($totalAmount);
                $amountDifference = floatval($pendingAmount) - floatval($studentFeePaymentDetailsModel->getAmount());
                if ($amountDifference >= 0) {
                  $studentFeePaymentDetailsModel->setId($id);
                  $studentFeePaymentDetailsModel->setStudent_Fees_Details_Id($feePaymentDetails[0]['student_fees_details_id']);
                  $this->studentAdmissionService->saveStudentFeePaymentDetails($studentFeePaymentDetailsModel);
                  if (intval($amountDifference) === 0) {
                    $this->studentAdmissionService->updateStudentFeeDetailsStatus($feePaymentDetails[0]['student_fees_details_id'], $status = 'Completed'); // All due amount is 0 then change the status to completed
                  } else if (intval($amountDifference) > 0) {
                    $this->studentAdmissionService->updateStudentFeeDetailsStatus($feePaymentDetails[0]['student_fees_details_id'], $status = 'Pending'); // Due amount is greater than 0 then change the status to pending
                  }
                  $this->auditTrailService->saveAuditTrail("UPDATE", "Update Student Fees Payment Details", "ALL", "SUCCESS");
                  $this->flashMessenger()->addMessage(' Student Fees Payment Details are successfully updated');
                  return $this->redirect()->toUrl('/public/addstudentfees/makepayment/'.$this->my_encrypt($feePaymentDetails[0]['student_fees_details_id'], $this->keyphrase));
                }
                $message = 'Failure';
                $this->flashMessenger()->addMessage(' Fees exceeded the due amount');
              }
            } catch (\Exception $ex) {
              $message = 'Failure';
        ...
        return array(
          ...
          'feePaymentDetails' => $feePaymentDetails[0],
          'studentDetails' => $studentDetails,
    ...
    public function makepaymentAction()
    {
      ...
      if (is_numeric($id)) {
        ...
        $feeDetails = $this->studentAdmissionService->fetchStudentFeeDetails($id);
        $feePaymentDetails = $this->studentAdmissionService->fetchStudentFeePaymentDetails('student_fees_details_id', $id);
        $studentDetails = $this->studentAdmissionService->getStdPersonalDetails($feeDetails['student_id']);
        $paymentTypes = $this->studentAdmissionService->fetchPaymentTypes();
        $message = NULL;
        ...
        if ($request->isPost()) {
          ...
          if($form->isValid()) {
            try {
              $studentFeeDetails = $this->studentAdmissionService->listStudentFeeList($feeDetails['student_id'], $id)->toArray();
              if (isset($studentFeeDetails[0])) { // If student fee details found then get the pending amount
                $amountDifference = floatval($studentFeeDetails[0]['pending_amount']) - floatval($studentFeePaymentDetailsModel->getAmount());
                if ($amountDifference >= 0) {
                    $dublicate_val = $this->studentAdmissionService->checkChequeAndDraftNumExists($studentFeePaymentDetailsModel,$feeDetails['student_id']);
                    if($dublicate_val){
                        $studentFeePaymentDetailsModel->setStudent_Fees_Details_Id($id);
                        $this->studentAdmissionService->saveStudentFeePaymentDetails($studentFeePaymentDetailsModel);
                        if (intval($amountDifference) === 0) {
                          $this->studentAdmissionService->updateStudentFeeDetailsStatus($id, $status = 'Completed'); // All due amount is 0 then change the status to completed
                        } else if (intval($amountDifference) > 0) {
                          $this->studentAdmissionService->updateStudentFeeDetailsStatus($id, $status = 'Pending'); // Due amount is greater than 0 then change the status to pending
                        }
                        $this->auditTrailService->saveAuditTrail("INSERT", "Add Student Fees Payment Details", "ALL", "SUCCESS");
                        $this->flashMessenger()->addMessage(' Student Fees Payment Details are successfully added');
                        return $this->redirect()->toUrl('/public/addstudentfees/makepayment/'.$id_from_route);
                    }
                    $message = 'Failure';
                    if($studentFeePaymentDetailsModel->getpayment_type() == '2'){
                        $this->flashMessenger()->addMessage(' Cheque number already exists');
                    }elseif ($studentFeePaymentDetailsModel->getpayment_type() == '3')
                        $this->flashMessenger()->addMessage(' Demand Draft number already exists');
                }else{
                    $message = 'Failure';
                    $this->flashMessenger()->addMessage(' Fees exceeded the due amount');
                }
        ...
        return array(
          'form' => $form,
          'feeDetails' => $feeDetails,
          'feePaymentDetails' => $feePaymentDetails,
          'studentDetails' => $studentDetails,
          'paymentTypes' => $paymentTypes,
          'keyphrase' => $this->keyphrase,
          'message' => $message,
    ...
    public function bulkGenerateFeesAction() {
        $this->loginDetails();
      
        $organizationSelectData = [];
        $form = new AddBulkFeeForm();
        $message = NULL;
        
        $studentFeeStructureList = $this->studentAdmissionService->getStudentFeeStructureDetails($this->organisation_id);
        ...
        if ($request->isPost()) {
          ...
          if($form->isValid()) {
            $data = $request->getPost()->toArray();
            try {
              $results = $this->studentAdmissionService->generateBulkStudentFees($data['organisation_id'], $data['structure_id'], $data['due_date']);
              $this->auditTrailService->saveAuditTrail("INSERT", "Add Student Fees Payment Details", "ALL", "SUCCESS");
              if ($results) {
                $this->flashMessenger()->addMessage(' Student fees payment details are successfully added');
              } else if ($results === 0) {
                $message = 'Failure';
                $this->flashMessenger()->addMessage(' Fees are already added in all students');
              } else {
                $message = 'Failure';
                $this->flashMessenger()->addMessage(' Some of the student fees payment details are already added');
              }
            } catch (\Exception $ex) {
              $message = 'Failure';
              $this->flashMessenger()->addMessage($ex->getMessage());
        ...
        return array(
          'form' => $form,
          'studentFeeStructureList' => $studentFeeStructureList,
          'message' => $message,
    ...
    public function deletepaymentAction()
    {
      ...
      if (is_numeric($id)) {
        $feePaymentDetails = $this->studentAdmissionService->fetchStudentFeePaymentDetails('id', $id);
        $isDeleted = $this->studentAdmissionService->deleteStudentFeePaymentDetails($id);
        $this->auditTrailService->saveAuditTrail("DELETE", "Delete Student Fees Payment Details", "ALL", "SUCCESS");
        $this->flashMessenger()->addMessage(' Student Fees successfully deleted');
        return $this->redirect()->toUrl('/public/addstudentfees/makepayment/'.$this->my_encrypt($feePaymentDetails[0]['student_fees_details_id'], $this->keyphrase));
      }
      return $this->redirect()->toRoute('selffinancedstudentlists');
    }
