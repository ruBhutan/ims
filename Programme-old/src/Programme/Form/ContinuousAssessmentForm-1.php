...
class ContinuousAssessmentForm extends Form
{
	protected $username;
       ...
       $this->add(array(
		 ...
		 $this->add(array(
           ...
           'type'=> 'select',
             ...
             'attributes' => array(
                  'class' => 'form-control',
				  'id' => 'selectProgrammeAssessmentComponent',
				  'required' => true,
		 ...
		 $this->add(array(
           'name' => 'academic_modules_allocation_id',
            'type'=> 'select',
             ...
             'attributes' => array(
                  'class' => 'form-control',
				  'id' => 'selectModuleAssessmentComponent',
		 ...
		 $this->add(array(
           'name' => 'assessment_component_id',
            'type'=> 'select',
             ...
             'attributes' => array(
                  'class' => 'form-control',
				  'id' => 'selectAssessmentComponentType',
				  'options' => array(),
             ),
         ));
	...
	private function createAcademicModule()
    {
		...
		$dbAdapter1 = $this->serviceLocator->get('Zend\Db\Adapter\Adapter');
        $sql       = 'SELECT `t1`.`id` AS `id`, `t2`.`module_coordinator` AS `module_coordinator` FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_coordinators` AS `t2` ON `t1`.`academic_modules_id` = `t2`.`id` WHERE t1.academic_year = '. $academic_year;
        $statement = $dbAdapter1->query($sql);
        $result    = $statement->execute();
        $selectData = array();
