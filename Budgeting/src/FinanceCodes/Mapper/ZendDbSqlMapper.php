	...
	*/
	...
	public function getUserDetailsId($username, $usertype)
	{
		...
		if($usertype == '1'){
			...
			$select->columns(array('id', 'first_name', 'middle_name', 'last_name', 'organisation_id', 'departments_id', 'profile_picture'));
	...
	*/
	public function findAll($tableName)
	{
		...
		if($tableName == 'chart_of_accounts'){
			...
			$select->join(array('t2' => 'accounts_group_head'),
				't2.id = t1.accounts_group_head_id', array('group_head'));
			$select->limit(15);
		}else if($tableName == 'object_code'){
			$select->from(array('t1' => $tableName));
			$select->join(array('t2' => 'broad_head_name'),
				't2.id = t1.broad_head_name_id', array('broad_head_name'));
			$select->limit(15);
		}
		else{
