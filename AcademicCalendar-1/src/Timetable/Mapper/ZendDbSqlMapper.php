...
class ZendDbSqlMapper implements TimetableMapperInterface
{
	...
	*/
	...
	public function getTutorTimetable($employee_details_id, $status)
	{
		...
		$timetable = array();
		$tutor_and_modules = array();
		$academic_timetable_ids = array();
		//get the organisation id
		$organisationID = $this->getOrganisationId($employee_details_id);
		foreach($organisationID as $organisation){
			$organisation_id = $organisation['organisation_id'];
		...
		$select1->from(array('t1' => 'academic_module_tutors')) 
                            ->columns(array('section','academic_modules_allocation_id'))
							->join(array('t2' => 'academic_modules_allocation'), 
                                't1.academic_modules_allocation_id = t2.id', array('id'));
		$select1->where->like('t1.module_tutor','%'.$employee_details_id.'%');
		$select1->where(array('t2.academic_year' => $academic_year));
		$select1->where(array('t2.academic_session' => $semester_type));
		...
		foreach($resultSet1 as $set1){
			$tutor_and_modules[][$set1['academic_modules_allocation_id']] = $set1['section'];
		}
		
		//for reference $select and $select2 could be combined as one
		foreach($tutor_and_modules as $key => $value){
			foreach($value as $k2 => $v2){
				$sql2 = new Sql($this->dbAdapter);
				$select2 = $sql2->select();
		
				$select2->from(array('t1' => 'academic_timetable')) 
									->columns(array('id'));
				$select2->where(array('t1.group' => $v2));
				$select2->where(array('t1.academic_modules_allocation_id' => $k2));
				$select2->where(array('t1.status' => $status));
				...
				foreach($resultSet2 as $set2){
					array_push($academic_timetable_ids, $set2['id']);
				}
			}
		}
		if($academic_timetable_ids){
			$sql = new Sql($this->dbAdapter);
			$select = $sql->select();
			...
			$select->from(array('t1' => 'academic_module_tutors')) 
								->columns(array('academic_modules_allocation_id', 'section'))
								->join(array('t2' => 'academic_modules_allocation'), 
									't1.academic_modules_allocation_id = t2.id', array('academic_year','module_code', 'module_title'))
								->join(array('t3' => 'academic_timetable'), 
									't2.id = t3.academic_modules_allocation_id')
								->join(array('t4' => 'programmes'), 
									't2.programmes_id = t4.id', array('programme_code'))
								->join(array('t5' => 'student_section'), 
									't3.group = t5.id', array('section'));
			$select->where(array('t2.academic_year' => $academic_year));
			$select->where(array('t2.academic_session' => $semester_type));
			$select->where(array('t3.status' => $status));
			$select->where->like('t1.module_tutor','%'.$employee_details_id.'%');
			$select->where(array('t3.id' => $academic_timetable_ids));
	...
	*/
	...
	public function crosscheckTiming($timetableObject)
	{
		$timetableData = $this->hydrator->extract($timetableObject);
		unset($timetableData['id']);
