	...
	public function addModuleAction()
    {
		...
		$request = $this->getRequest();
		 if ($request->isPost()) {
			   ...
			   //var_dump($programme_module_check); die();

			   if ($programme_module_check){
					$this->flashMessenger()->addMessage('Module with similar Module Code already exists for the selected Programme');
					 $message = 'Failure';
			   } else {
				   if ($form->isValid()) {
						try {
	         ...
	         if ($request->isPost()) {
	             ...
	             if ($form->isValid()) {
	                 try {
						 ...
						 if((($marks_data['marks']/$markDetails['weightage'])*100)<40){
							$marks_data['status'] = 'Re-assessment';	
						 } else {
						 	$marks_data['status'] = 'Pass';
						 }
