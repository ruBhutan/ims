...
class DisciplineController extends AbstractActionController
{
	...
	public function editDisciplinaryRecordAction()
    {
        ...
        if(is_numeric($id)){ 
			...
			$disciplinary_record_details = $this->disciplineService->getRecordedDisciplinaryRecordDetails($id);
			$student_details = array();
			foreach($disciplinary_record_details as $details){
			...
			if ($request->isPost()) {
				...
				if ($form->isValid()) {
					try {
						$this->disciplineService->saveDisciplinaryRecord($disciplineModel);
						$this->notificationService->saveNotification('Student Disciplinary Record', $student_details['student_id'], 'NULL', 'Student Disciplinary Record');
	                    $this->auditTrailService->saveAuditTrail("UPDATE", "Student Disciplinary Record", "ALL", "SUCCESS");
	                    $this->flashMessenger()->addMessage('Disciplinary Record was successfully edited');
						return $this->redirect()->toRoute('viewdisciplinaryrecord');
					}
					catch(\Exception $e) {
							die($e->getMessage());
			...
			return array(
				...
				'form' => $form,
				'disciplineCategory' => $disciplineCategory,
				'disciplinary_record_details' => $disciplinary_record_details,
			);
		}else{
			return $this->redirect()->toRoute('viewdisciplinaryrecord');
		}
