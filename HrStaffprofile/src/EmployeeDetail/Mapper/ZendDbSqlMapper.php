...
class ZendDbSqlMapper implements EmployeeMapperInterface
{
	...
	**/
	...
	public function rollBackEmployeeId($id)
	{
		...
		$select->from(array('t1' => 'new_employee_details'),array('cid'))
				->join(array('t2' => 'job_applicant'), 
						't2.cid = t1.cid', array('id'))
				->where('t1.id = ' .$id); 
                //$select->where('t1.cid = t2.cid');
		...
		foreach ($resultSet as $row)
		{
			...
			if ($cid != null) 
			{
						...
						if ($resultDelete) 
						{
								 $sqlUpdate = new Sql($this->dbAdapter);
								 $update =  new Update('emp_job_applications');

								 $update->set(array('status' => 'Selected'));

								 $update->where ('job_applicant_id='.$row['id']);

								 $stmtUpdate = $sqlUpdate->prepareStatementForSqlObject($update);
	...
	// Get email IDs of the respective college to be forwarded email for resetting employee details to generate ID

	public function getEmailtoSend($org_id)
	{
		$sql = new Sql($this->dbAdapter);
	...
	//	var_dump($resource);
		...
		{
			//echo "OrgID:".$result['organisation_id'];
			return ($this->getEmailAddress($result['organisation_id']));
			//$this->getEmailAddress($result['organisation_id']);
	...
	/*
	*   Get Email Address of the administrator of the HR
	*/
	
	private function getEmailAddress($org_id)
	{

		$sql = new Sql($this->dbAdapter);

		$email = $sql->select();
		//SELECT e.email as email FROM user_routes u1, user_role ur, user_role_routes ul, users u, employee_details e   
		//where  u1.id = ul.user_routes_id 
		//and ul.user_role_id= ur.id 
		//and ur.rolename = u.role 
		//and u.username = e.emp_id 
		//and u1.route_details='addnewemployee' and ur.organisation_id=3		


		$email->from (array('u1'=>'user_routes'))
			  ->join(array('u2'=> 'user_role_routes'),
						'u1.id = u2.user_routes_id')
			  ->join(array('u3'=> 'user_role'),
					 'u2.user_role_id = u3.id')
			  ->join(array('u4' => 'users'),
			         'u3.rolename=u4.role')
			  ->join(array('u5' => 'employee_details'),
			         'u4.username = u5.emp_id', array('email'));

		$email->where (array('u1.route_details=?'=>'addnewemployee', 'u3.organisation_id=?' => $org_id));
		
		$stmtemail = $sql->prepareStatementForSqlObject($email);

		$resource  = $stmtemail->execute();
	...
	// To send email to sender as well
	public function getEmailAddressForSender($username)
	{
                ...
                $select->where(array('t1.emp_id LIKE  ?' => $username));
