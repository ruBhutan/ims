...
class StaffPortalController extends AbstractActionController
{
    ...
    public function staffDashboardAction()
     ...
     public function individualAssessmentDetailAction()
     { 
        ...
        if(is_numeric($id)){ 
            ...
            $academic_assessment_details = $this->staffService->getAcademicAssessmentDetails($assessment_component_id, $this->employee_details_id, $this->organisation_id);

            return new ViewModel(array(
                'academic_assessment_details' => $academic_assessment_details,
            ));
        } else {
    ...
    public function ajaxGetSubMenusAction()
    {
        ...
        //$sql = "SELECT `t1`.* FROM `user_menu` AS `t1` WHERE `t1`.`user_menu_id` = '$parentValue1' AND `menu_name` NOT IN(SELECT `t2`.`route_name` AS `route_name` FROM `user_routes` AS `t2` WHERE `t2`.`parent_menu_id` = '$parentValue1')";
    ...
    public function ajaxGetRouteDetailsAction()
    {
        ...
        $sql = "SELECT `t1`.* FROM `route_list` AS `t1` WHERE `t1`.`user_module_id` = '$parentValue' AND `route_details` NOT IN(SELECT `t2`.`route_details` FROM `user_routes` AS `t2` WHERE `t2`.`route_category` = '$parentValue')";
    ...
    */
    ...
    public function ajaxModuleNameAction()
    {
        ...
        if($semester == 'Autumn'){ 
            $sql = "SELECT id, module_title, module_code FROM academic_modules_allocation WHERE academic_year = '$academic_year'  AND academic_session = '$semester' AND programmes_id='$parentValue'";
        }
        else{
            $sql = "SELECT id, module_title, module_code FROM academic_modules_allocation WHERE academic_year = '$academic_year'  AND academic_session = '$semester' AND programmes_id='$parentValue'";
        }
    ...
    */
    ...
    public function ajaxPayScaleAction()
    {
        ...
        foreach ($result as $res) {
            $selectTwoData[$res['id']] = $res['minimum_pay_scale'].'-'.$res['increment'].'-'.$res['maximum_pay_scale'];
        }
   ...
   // For listing staff list to issue adhoc goods based on department
    ...
    public function ajaxAdhocGoodsIssueItemDetailsAction()
    {
        ...
        $sql       = "SELECT `t1`.`id` AS `id`, `t1`.`item_in_stock` AS `item_in_stock`, `t1`.`item_received_date` AS `item_received_date`,`t1`.`item_received_purchased_id` AS `item_received_purchased_id`,`t1`.`item_specification` AS `item_specification`,`t2`.`item_name` AS `item_name` FROM `goods_received` AS `t1` INNER JOIN `item_name` AS `t2` ON `t1`.`item_name_id` = `t2`.`id` INNER JOIN `item_sub_category` AS `t3` ON `t3`.`id` = `t2`.`item_sub_category_id` WHERE `t2`.`id`='$parentValue1' AND `t1`.`item_in_stock` > '0' AND `t3`.`organisation_id`= '$this->organisation_id' ORDER BY `t1`.`id` ASC";
        $statement = $dbAdapter->query($sql);
        $result    = $statement->execute();
        $selectTwoData = array();
        $selectTwoData = array('0' => 'THERE IS NO ITEM IN STOCK');
       ...
       foreach ($result as $res) {
            unset($selectTwoData[0]);
            $selectTwoData[$res['id']] = $res['item_received_purchased_id']. '-'.$res['item_name']. ' ('.$res['item_in_stock'].' Qty)'. ' - '.$res['item_received_date']. ' (' .$res['item_specification']. ')';
        }
   ...
   //ajax actions for programme module
     ...
     public function ajaxAssessmentSectionAction()
    {
        ...
        $sql       = "SELECT `t1`.*, `t2`.`academic_modules_allocation_id` AS `academic_modules_allocation_id` FROM `student_section` AS `t1` INNER JOIN `academic_module_tutors` AS `t2` ON `t1`.`id` = `t2`.`section` WHERE `t2`.`academic_modules_allocation_id` ='$parentValue' AND `t2`.`module_tutor` LIKE '$this->username'";
        $statement = $dbAdapter1->query($sql);
        $result    = $statement->execute();
        $selectTwoData = array();
    ...
    */
    ...
    public function ajaxElectiveModuleNameAction()
    {
        ...
        $dbAdapter1 = $this->serviceLocator->get('Zend\Db\Adapter\Adapter');
    $sql1       = "SELECT `t1`.`id`, `t1`.`module_title`, `t1`.`module_code`, `t2`.`module_tutor` AS `module_tutor` FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_tutors` AS `t2` ON `t1`.`id` = `t2`.`academic_modules_allocation_id` WHERE `t1`.`module_type` = 'Elective' AND `t1`.`programmes_id`='$parentValue' AND `t1`.`academic_year`='$academic_year' AND `t1`.`academic_session`='$semester' AND `t2`.`module_tutor`='$this->username'";

        //$dbAdapter1 = $this->serviceLocator->get('Zend\Db\Adapter\Adapter');
        //$sql1     = "SELECT `t1`.`id`, `t1`.`module_title`, `t1`.`module_code`, `t2`.`module_tutor` AS `module_tutor` FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_tutors` AS `t2` ON `t1`.`id` = `t2`.`academic_modules_allocation_id` WHERE `t1`.`module_type` = 'Elective' AND `t1`.`programmes_id`='$parentValue' AND `t2`.`module_tutor`='$this->username'";
    $statement1 = $dbAdapter1->query($sql1);
        $result1    = $statement1->execute();
        $selectTwoData = array();
        $selectTwoData[0]="Please Select a Module";
    ...
    public function ajaxElectiveSectionAction()
    {
        ...
        $sql       = "SELECT `t1`.`module_code`, `t2`.`section` AS `section`, `t3`.* FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_tutors` AS `t2` ON `t2`.`academic_modules_allocation_id` = `t1`.`id` INNER JOIN `student_section` AS `t3` ON `t2`.`section` = `t3`.`id` WHERE `t2`.`academic_modules_allocation_id`='$parentValue' AND `t2`.`module_tutor`='$this->username'";
        
        $statement = $dbAdapter->query($sql);
        $result    = $statement->execute();
        $selectTwoData = array();
        $selectTwoData[0]="Please Select a Section";
        foreach ($result as $res) {
            $selectTwoData[$res['id']] = $res['section'];
        }
    ...
    public function ajaxModuleAttendanceRecordAction()
    {
        ...
        //$academic_event_details = $this->getSemester($academic_session);
     ...
     public function ajaxModuleNameForDeleteAttendanceAction()
    {
      $parentValue = $_POST['value'];
       ...
       /* $sql = 'SELECT `t1`.`id` AS `id`, `t1`.`module_title` AS `module_title`, `t1`.`module_code` AS `module_code`, `t2`.`programmes_id` AS `programmes_id`, `t3`.`module_tutor` AS `module_tutor`, `t4`.`programme_code` AS `programme_code` FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_tutors` AS `t3` ON `t3`.`academic_modules_allocation_id` = `t1`.`id` INNER JOIN `academic_modules` AS `t2` ON `t1`.`academic_modules_id` = `t2`.`id` INNER JOIN `programmes` AS `t4` ON `t2`.`programmes_id` = `t4`.`id` WHERE t1.academic_year = "'.$academic_year.'" AND t1.academic_session = "'.$semester.'" AND t3.module_tutor LIKE "%'.$this->username.'%" AND t1.programmes_id = "'.$parentValue'"';
        ...
        $sql       = "SELECT `t1`.`id` AS `id`, `t1`.`module_title` AS `module_title`, `t1`.`module_code` AS `module_code`, `t2`.`programmes_id` AS `programmes_id`, `t3`.`module_tutor` AS `module_tutor`, `t4`.`programme_code` AS `programme_code` FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_tutors` AS `t3` ON `t3`.`academic_modules_allocation_id` = `t1`.`id` INNER JOIN `academic_modules` AS `t2` ON `t1`.`academic_modules_id` = `t2`.`id` INNER JOIN `programmes` AS `t4` ON `t2`.`programmes_id` = `t4`.`id` WHERE `t1`.`programmes_id` ='$parentValue' AND `t1`.`academic_year` = '$academic_year' AND `t1`.`academic_session` = '$semester' AND `t3`.`module_tutor` = '$this->username'";
    ...
    */
    ...
    $dbAdapter = $this->serviceLocator->get('Zend\Db\Adapter\Adapter');
/*  $sql       = "SELECT `t1`.`module_tutor` AS `module_tutor`, `t2`.`*`  FROM `academic_module_tutors` AS `t1` INNER JOIN `student_section` AS `t2` ON `t1`.`section` = `t2`.`id` WHERE `module_tutor` = '$this->username' AND `t1`.`academic_modules_allocation_id`='$parentValue'";*/
        
    $sql       = "SELECT `t1`.`academic_modules_id` AS `academic_modules_id`, `t2`.`module_tutor` AS `module_tutor`, `t3`.`id`,`t3`.`section`  FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_tutors` AS `t2` ON `t1`.`id` = `t2`.`academic_modules_allocation_id` INNER JOIN `student_section` AS `t3` ON `t2`.`section` = `t3`.`id` WHERE `module_tutor` = '$this->username' AND `t1`.`id`='$parentValue'";
    ...
    public function ajaxAssignAssessmentAction()
    {
        ...
        $sql       = "SELECT id, assessment FROM assessment_component where academic_modules_allocation_id='$parentValue' and weightage != '0'";
        $statement = $dbAdapter1->query($sql);
        $result    = $statement->execute();
        $selectTwoData = array();
        $selectTwoData[0]="Please Select Assessment Type";
