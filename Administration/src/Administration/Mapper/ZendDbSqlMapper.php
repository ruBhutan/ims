...
class ZendDbSqlMapper implements AdministrationMapperInterface
{
	...
	*/
	...
	public function saveRoutes(UserRoutes $moduleObject)
	{
		...
		$moduleData['route_Name'] = $this->getModuleRouteName($moduleData['user_Sub_Menu_Id']);
	...
	public function getModuleRouteName($sub_menu_id)
	{
		$sql = new Sql($this->dbAdapter);
		$select = $sql->select();

		$select->from(array('t1' => 'user_menu'))
		       ->where(array('t1.id' => $sub_menu_id));
