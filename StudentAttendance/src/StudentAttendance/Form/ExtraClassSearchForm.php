...
class ExtraClassSearchForm extends Form
{
	protected $username;
	...
	private function createAcademicModule()
    {
		...
		/*
        $sql       = 'SELECT `t1`.`id` AS `id`, `t1`.`module_title` AS `module_title`, `t2`.`programmes_id` AS `programmes_id`, `t3`.`module_tutor` AS `module_tutor`, `t4`.`programme_code` AS `programme_code` FROM `academic_modules_allocation` AS `t1` INNER JOIN `academic_module_tutors` AS `t3` ON `t3`.`academic_modules_allocation_id` = `t1`.`id` INNER JOIN `academic_modules` AS `t2` ON `t1`.`academic_modules_id` = `t2`.`id` INNER JOIN `programmes` AS `t4` ON `t2`.`programmes_id` = `t4`.`id` WHERE t1.academic_year = "'.$academic_year.'" AND  t3.module_tutor LIKE "%'.$this->username.'%"';
		*/
