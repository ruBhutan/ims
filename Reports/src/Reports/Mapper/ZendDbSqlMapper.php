...
class ZendDbSqlMapper implements ReportsMapperInterface
{
	...
	*/
	...
	public function getHrReport($report_details, $organisation_id)
	{
		...
		switch($report_name){
			...
			case "staff_cancelled_leave":
				$hr_reports = $this->getStaffCancelledLeave($report_details['date'], $organisation_id);
				break;
			...
			case "overall":
				$hr_reports = $this->getStaffOverall($organisation, $year=1);
				break;

			case "emptypestats":
				$hr_reports = $this->getEmpTypeStats($organisation, $year=1);
				break;
		}
	...
	*/
	...
	{ 
		...
		else if($report_type == 'student_list_for_academics') {
			if($section == 'All'){
				$select->from(array('t1' => 'student_semester_registration'),array('id'))
	               ...
	               ->where(array('t1.academic_year' => $academic_year, 't1.student_section_id is NOT NULL', 't2.programmes_id' => $programmes_id, 't2.student_status_type_id' => '1'))
	               ->order('t5.id ASC, t1.semester_id DESC, t2.enrollment_year DESC, t2.student_id ASC')
	               ->where->notLike('t2.student_id', "TEMP_%");
			}else{
				$select->from(array('t1' => 'student_semester_registration'), array('semester_id'))
	               ->join(array('t2' => 'student'),
	                    't2.id = t1.student_id', array('first_name', 'middle_name', 'last_name', 'student_id'))
	               ->join(array('t3' => 'programmes'),
	                    't3.id = t2.programmes_id', array('programme_name'))
	               ...
	               ->join(array('t6' => 'programme_year'),
	                    't6.id = t1.year_id', array('year'))
	               ->where(array('t1.academic_year' => $academic_year, 't1.student_section_id is NOT NULL', 't2.programmes_id' => $programmes_id, 't2.student_status_type_id' => '1', 't1.student_section_id' => $section))
	               ->order('t1.semester_id DESC, t2.enrollment_year DESC, t2.student_id ASC')
	               ->where->notLike('t2.student_id', "TEMP_%");
			}
...
/**
	 *    To get all segregation of staff as required by HR
	 * 
	 */
	public function getStaffOverall($organisation, $year)
	{
		$hr_data = array();
		$hr_reports = array();
		...
		$select = $sql->select();
		$select->from(array('t1' => 'employee_details'))
						->columns(array('emp_id', 'emp_type', 'nationality', 'occupationaCount' => new Expression ('COUNT(t9.major_occupational_group)')
						,'genderCount' => new Expression ('COUNT(t3.gender)'),'typeCount' => new Expression ('COUNT(t2.employee_type)')
						...
						->join(array('t10'=>'nationality'),
							't10.id=t1.nationality', array('nationality'));
		$select->where(array('t1.emp_resignation_id = ? ' => '0'));
		//$select->order('t4.id ASC');
		$select->group(array('t4.organisation_name','t9.major_occupational_group', 't3.gender','t4.abbr','t2.employee_type'));
		if($organisation != 0){
		...
		$resultSet->initialize($result);
		foreach($resultSet as $set){
			//$hr_data[$set['abbr']][$set['major_occupational_group']][$set['nationality']][$set['gender']][$set['emp_id']] = $set['emp_id'];
		
		//	echo $set['organisation_name'].'--'.$set['major_occupational_group'].'--'.$set['nationality'] .'--'.$set['employee_type'].'--'.$set['genderCount'].'<br />';

			//$hr_reports[$set['organisation_name']][$set['major_occupational_group']][$set['employee_type']$set['typeCount'].'Nationality:'.$set['nationality']] = ;
			if($set['nationality'] == 'Bhutanese')
            {
			     $hr_reports[$set['organisation_name']][$set['major_occupational_group']] ['national'][$set['gender']] += $set['genderCount'];

				 //echo "Org:".$set['organisation_name']."--Occupation:".$set['major_occupational_group'] .'<br />';
			}

		    else
			    
			     $hr_reports[$set['organisation_name']][$set['major_occupational_group']] ['international'][$set['gender']] += $set['genderCount'];

			if ($set['employee_type'] == 'Regular') {

			               $hr_reports[$set['organisation_name']] ['regular'] += $set['typeCount'];
							//echo $set['typeCount']."<br />";
						}

			else
			               $hr_reports[$set['organisation_name']] ['fixed'] += $set['typeCount'];          
		}
 ...
 */
	...
	public function getGender()
	{
		$sql = new Sql($this->dbAdapter);
		$gender = $sql->select();
		$gender->from(array('g1' => 'gender'))
			    ->columns (array('id as genId', 'gender'));

		$stmtgender = $sql->prepareStatementForSqlObject($gender);
		$resultgender = $stmtgender->execute();
		
		$resultSetgender = new ResultSet();

		$resultSetgender->initialize($resultgender); 

		$resultSetgender->buffer();
	...
	*/
	public function getEmptype()
	{
		...
		$sql = new Sql($this->dbAdapter);
		$resource = $sql->select();
		$resource->from(array('t1' => 'employee_type'))
			    ->columns (array('id as empId', 'employee_type'));

		$stmtresource = $sql->prepareStatementForSqlObject($resource);
		$resultresource = $stmtresource->execute();
		
		$resultSetresource = new ResultSet();

		$resultSetresource->initialize($resultresource); 

		$resultSetresource->buffer();

		return $resultSetresource;

	}
//Get overall employee type


	public function getEmpTypeStats($organisation, $year)
	{
		$hr_data = array();
		$hr_reports = array();
		...
		$select = $sql->select();
		$select->from(array('t1' => 'employee_details'))
						->columns(array('emp_id', 'emp_type','typeCount' => new Expression ('COUNT(t2.employee_type)')
		...
		$select->where(array('t1.emp_resignation_id = ? ' => '0'));
		//$select->order('t4.id ASC');
		$select->group(array('t3.organisation_name','t3.abbr','t2.employee_type'));
		if($organisation != 0){
			$select->where(array('t1.organisation_id' =>$organisation));
		}
		$stmt = $sql->prepareStatementForSqlObject($select);
		...
		$resultSet->initialize($result);
		foreach($resultSet as $set){
			$hr_reports[$set['organisation_name']] [$set['employee_type']] += $set['typeCount'];
	...
	*/
	...
	/*
	 * Get the List of Staff that have cancelled their approved leave
	 */
	
	public function getStaffCancelledLeave($date, $organisation)
	{
		$from_date = date("Y-m-d", strtotime(substr($date,0,10)));
		$to_date = date("Y-m-d", strtotime(substr($date,13,10)));
		
		$cancelled_leave_report = array();
		...
		if($organisation != 1){
			...
			$select->from(array('t1' => 'emp_cancelled_leave'))
				...
				->join(array('t3'=>'departments'),
					't2.departments_id = t3.id',array('department_name'))
				->join(array('t4'=>'emp_leave'),
					't1.emp_leave_id = t4.id',array('leave_status'))
				->join(array('t5'=>'emp_leave_category'),
					't4.emp_leave_category_id = t5.id',array('leave_category'))
				->join(array('t6'=>'employee_details'),
					't1.cancelled_by = t6.id',array('sub_first_name'=>'first_name', 'sub_middle_name'=>'middle_name', 'sub_last_name'=>'last_name'))
				->join(array('t7'=>'organisation'),
					't2.organisation_id = t7.id',array('abbr'));
			//$select->where(array('t2.emp_resignation_id = ? ' => '0'));
			//$select->where(array('t1.leave_status' => 'Approved'));
			$select->where(array('t1.from_date >= ? ' => $from_date, 't1.to_date <= ? ' => $to_date));
			$select->where(array('t2.organisation_id' => $organisation));
			...
			foreach($resultSet as $set){
				$cancelled_leave_report[] = $set;
			}
			return $cancelled_leave_report;
		}else {
			$select = $sql->select();
			$select->from(array('t1' => 'emp_cancelled_leave'))
				->join(array('t2'=>'employee_details'),
			...
			$select1 = $sql->select();
			$select1->from(array('t1' => 'emp_cancelled_leave'))
				->join(array('t2'=>'employee_details'),
					't1.employee_details_id = t2.id',array('emp_id','first_name', 'middle_name', 'last_name'))
				->join(array('t3'=>'departments'),
					't2.departments_id = t3.id',array('department_name'))
				...
				->join(array('t7'=>'organisation'),
					't2.organisation_id = t7.id',array('abbr'))
				->join(array('t8' => 'users'),
					't8.username = t2.emp_id', array('role'));
			//$select1->where(array('t2.emp_resignation_id = ? ' => '0'));
			//$select1->where(array('t1.leave_status' => 'Approved'));
			$select1->where(array('t1.from_date >= ? ' => $from_date, 't1.to_date <= ? ' => $to_date));
			$select1->where->notLike('t2.organisation_id','1');
			$select1->where->like('t8.role', "%_PRESIDENT");
			...
			foreach($resultSet1 as $set1){
				$cancelled_leave_report[] = $set1;
			}

			//var_dump($cancelled_leave_report); die;
			return $cancelled_leave_report;
		}	
	}
	...
	{ 
		$student_reports = array();
                ...
                switch($report_name){
					...
					case "student_by_programme_incampus":
                        $student_reports = $this->getStudentByProgrammeInCampus($organisation, $year);
                        break;
					...
					case "student_by_programme_offcampus":
                        $student_reports = $this->getStudentByProgrammeOffCampus($organisation, $year);
                        break;
                    ...
                    case "suspended_students":
                        $student_reports = $this->getSuspendedStudents($organisation, $year);
                        break;
                    case "terminated_students":
                        $student_reports = $this->getTerminatedStudents($organisation, $year);
                        break;
                    case "withdrawn_students":
                        $student_reports = $this->getwithdrawnStudents($organisation, $year);
                        break;
					
					case "student_by_dzongkhag":
                        $student_reports = $this->getStudentByDzongkhag($organisation, $year);
                        break;
					
					case "student_by_nationality":
                        $student_reports = $this->getStudentByNationality($organisation, $year);
                        break;
					
					case "student_by_religion":
                        $student_reports = $this->getStudentByReligion($organisation, $year);
                        break;
					
					case "student_by_gender":
                        $student_reports = $this->getStudentByGender($organisation, $year);
                        break;
					
					case "student_by_bloodgroup":
                        $student_reports = $this->getStudentByBloodGroup($organisation, $year);
                        break;
                }
	...
	*/
	...
	/*
	* Student By Programme Enrolled in Campus
	*/
	
	public function getStudentByProgrammeInCampus($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
		...
		$resultSet->initialize($result);
		foreach($resultSet as $set){
			$student_data[$set['abbr']][$set['programme_name']][$set['year_id']][$set['gender']][$set['student_id']] = $set['student_id'];
		}
		foreach($student_data as $key => $value){
			foreach($value as $key2 => $value2){
				foreach($value2 as $key3 => $value3){
	...
	*/
	...
	public function getStudentByProgrammeOffCampus($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
		$semester = NULL;
	...
	*/
	...
	public function getSuspendedStudents($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
		$max_programme_duration = $this->getMaxProgrammeDuration();
	...
	*/
	...
	public function getTerminatedStudents($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
		$max_programme_duration = $this->getMaxProgrammeDuration();
	...
	*/
	...
	public function getWithdrawnStudents($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
		$max_programme_duration = $this->getMaxProgrammeDuration();
	...
	*/
	...
	public function getStudentByDzongkhag($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
		...
		$resultSet->initialize($result);
		foreach($resultSet as $set){
			$student_data[$set['abbr']][$set['dzongkhag_name']][$set['gender']][$set['student_id']] = $set['student_id'];
		}
		foreach($student_data as $key => $value){
			foreach($value as $key2 => $value2){
				foreach($value2 as $key3 => $value3){
	...
	*/
	...
	public function getStudentByNationality($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
		...
		$resultSet->initialize($result);
		foreach($resultSet as $set){
			$student_data[$set['abbr']][$set['country']][$set['gender']][$set['student_id']] = $set['student_id'];
		}
		foreach($student_data as $key => $value){
			foreach($value as $key2 => $value2){
				foreach($value2 as $key3 => $value3){
	...
	*/
	...
	public function getStudentByReligion($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
	...
	*/
	...
	public function getStudentByGender($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
	...
	*/
	...
	public function getStudentByBloodGroup($organisation, $year)
	{
		$student_data = array();
		$student_reports = array();
