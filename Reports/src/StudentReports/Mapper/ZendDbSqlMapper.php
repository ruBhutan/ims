...
class ZendDbSqlMapper implements StudentReportsMapperInterface
{
	...
	*/
	...
	{ 
		$student_reports = array();
			...
			break;
			case "student_by_overall":
				$student_reports = $this->getStudentOverAllReport($organization);
				break;
                }
	...
	*/
	...
	public function getStudentByProgrammeInRUB($organisation)
	{
        ...
        */
        ...
        private function getStudentOverAllReport($organization)
        {
                $student_data = array();
                ...
                $select->from (array('t1' => 'organisation'))
                                ...
                                ->columns(array('id as orgID','abbr', 'organisation_name'))
                                ->join(array('t2' => 'programmes'),
                                  't2.organisation_id = t1.id', array('id','programme_name'));
                ...
                $scho = $sql->select();
                $scho->from(array('s1' => 'student_type'))
                         ->columns (array('id as schoId', 'student_type'));

                $stmtscho = $sql->prepareStatementForSqlObject($scho);
		$resultscho = $stmtscho->execute();
	        $resultSetScho = new ResultSet();
                $resultSetScho->initialize($resultscho);
                $resultSetScho->buffer();
                ...
                foreach ($resultSet2 as $row)
                {
                        ...
                        foreach ($year_id as $yrow => $key)



                           foreach ($resultSetScho as $scholarships)
                           {
                                        foreach ($resultSetgender as $gen)
                                        {

                                                $student= $sql->select();

                                                $student->from(array('t1' => 'student'))
                                                ...
                                                $student->where(array('t5.year_id' =>$yrow));

                                                $student->where(array('t1.gender' =>$gen['genId']));

						$student->where(array('t1.programmes_id' => $row['id']));
					        $student->where(array('t1.scholarship_type' => $scholarships ['schoId']));

                                                $select->where(array('t1.student_status_type_id' => $student_status));

                                                $stmtstudent = $sql->prepareStatementForSqlObject($student);

                                                $resultStudent = $stmtstudent->execute();

                                                foreach ($resultStudent as $count)
                                                {

                                                        if($count['num'] > 0)

                                                                $studentRecords[$row['orgID']][$row['id']][$yrow][$scholarships ['schoId']][$gen['genId']] = $count['num'];
                                                }
                                        }
	...
	// max duration
        ...
        public function getMaxDuration()
        {
                ...
                $select = $sql->select();
                $select->from(array('t1' => 'programmes'))
                                ->columns(array('programme_duration as max_duration','id'));
                ...
                foreach($resultSet as $length)
                {
                                $courseduration[$length['id']] = $length['max_duration'];
                }
	...
	public function getStudentByProgrammeInCampus($organisation)
	{
		...
		if($organisation == 0){
			...
			$academic_event_details = $this->getSemester($organisation);
	        $semester = $academic_event_details['academic_event'];
		}
		//$academic_year = $this->getAcademicYear($semester);
	...
	*/
	...
	public function getStudentByProgrammeEnrolled($organisation, $year)
	{
		...
		//student status
		// 1- Reported, 2,3 and 9- Suspended (disciplinary, medical and Other Reason)
        $student_status = array('1','2','3','9');
	...
	*/
	...
	public function getStudentByDzongkhag($organisation)
	{
		...
		$select->where(array("t1.date_of_birth >= '1998-01-01'"));
		$select->where(array("t1.date_of_birth <= '2007-12-31'"));
