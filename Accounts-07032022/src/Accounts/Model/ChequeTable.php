...
class ChequeTable extends AbstractTableGateway implements AdapterAwareInterface
{
	protected $table = 'accounts_cheque_book'; //tablename
	...
	{
		$where = ( is_array($param) )? $param: array('c.id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('c' => $this->table))
	...
	*/
	 ...
	 * @return Array
	 */
	public function getlocationDateWise($column,$userorg,$year,$month)
	{	
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
	 ...
	 * @return Array
	 */
	public function getMonthlyCQ($prefix_PO_code)
	{
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from($this->table);
		$select->where->like('cheque_code', $prefix_PO_code."%");
