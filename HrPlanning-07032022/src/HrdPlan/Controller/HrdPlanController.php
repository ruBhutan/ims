...
class HrdPlanController extends AbstractActionController
{
	...
	public function __construct(HrdPlanServiceInterface $hrdPlanService, NotificationServiceInterface $notificationService, AuditTrailServiceInterface $auditTrailService, $serviceLocator)
	{
		...
		*/
		...
		$this->userDetails = $this->hrdPlanService->getUserDetails($this->username, $this->usertype);
        $this->userImage = $this->hrdPlanService->getUserImage($this->username, $this->usertype);
	...
	public function approveHrdProposalAction()
    {
		...
		if(is_numeric($id)){
			try {
				 ...
				 $this->auditTrailService->saveAuditTrail("APPROVED", "HRD Plan", "ALL", "SUCCESS");
	...
	public function rejectHrdProposalAction()
    {
		...
		if(is_numeric($id)){
			try {
				 ...
				 $this->auditTrailService->saveAuditTrail("REJECTED", "HRD Plan", "ALL", "SUCCESS");
			 ...
			 if ($request->isPost()) {
				 ...
				 if ($form->isValid()) {
					 try {
						 $this->hrdPlanService->save($hrdPlanModel);
						 $this->flashMessenger()->addMessage('HRD Proposal was successfully edited');
						 $this->auditTrailService->saveAuditTrail("UPDATE", "HRD Plan", "ALL", "SUCCESS");
						 return $this->redirect()->toRoute('hrdproposal');
					 }
					 catch(\Exception $e) {
						 $message = 'Failure';
