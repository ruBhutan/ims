...
class ResponsibilitiesController extends AbstractActionController
{
	...
	public function __construct(ResponsibilitiesServiceInterface $responsibilityService, NotificationServiceInterface $notificationService, AuditTrailServiceInterface $auditTrailService, $serviceLocator)
	{
		...
		*/
		...
		$empData = $this->responsibilityService->getUserDetailsId($this->username, $tableName = 'employee_details');
		foreach($empData as $emp){
			$this->employee_details_id = $emp['id'];
			}

		if($this->employee_details_id == NULL){
			$studentData = $this->responsibilityService->getUserDetailsId($this->username, $tableName = 'student');
	...
	public function editStudentResponsibilityAction()
    {
        ...
        if(is_numeric($id)){ 
			...
			$stdResponsibilityDetails = $this->responsibilityService->getStudentResponsibilities($id);
	        ...
	        $request = $this->getRequest();
	         if ($request->isPost()) {
	             ...
	             if ($form->isValid()) { 
	                 try {
						 $this->responsibilityService->updateResponsibility($responsibilityModel);
	        ...
	        return array(
	        	'id' => $id,
				'form' => $form,
				'stdResponsibilityDetails' => $stdResponsibilityDetails,
				'responsibilityCategory' => $responsibilityCategory,
			);
        }
        else{
	...
	public function viewStudentResponsibilityDetailAction()
    {
        ...
        if(is_numeric($id)){ 
			...
			$studentResponsibilities = $this->responsibilityService->getStudentResponsibilities($id);
