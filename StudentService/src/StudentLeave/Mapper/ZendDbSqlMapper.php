...
class ZendDbSqlMapper implements StudentLeaveMapperInterface
{
	...
	*/
	...
	public function listAllLeave($status, $employee_details_id, $userrole, $organisation_id)
	{
		...
		$academic_session_dates = $this->getAcademicSessionDates($organisation_id); 
							...
							$select4->from(array('t1' => 'student_leave'))
							   ...
							   ->join(array('t3' => 'student'),
								...
								->join(array('t4' => 'employee_details'),
									't4.id = t1.approved_by', array('sfirst_name' => 'first_name', 'smiddle_name' => 'middle_name', 'slast_name' => 'last_name'))
								->join(array('t5' => 'gender'),
									't5.id = t3.gender', array('gender'));
							$select4->where(array('t1.from_date >= ? ' => $academic_session_dates['from_date'], 't1.from_date <= ? ' => $academic_session_dates['to_date'], 't1.leave_status' => $status, 't3.organisation_id' => $organisation_id));
							...
							$select4->from(array('t1' => 'student_leave'))
							   ...
							   ->join(array('t3' => 'student'),
								...
								->join(array('t4' => 'gender'),
									't4.id = t3.gender', array('gender'));
							$select4->where(array('t1.from_date >= ? ' => $academic_session_dates['from_date'], 't1.from_date <= ? ' => $academic_session_dates['to_date'], 't1.student_leave_category_id' => $key1, 't1.leave_status' => $status,'t3.organisation_id' => $organisation_id));
						}else{
							$select4->from(array('t1' => 'student_leave'))
							   ->join(array('t2' => 'student_leave_category'),
									't2.id = t1.student_leave_category_id', array('leave_category'))
	...
	public function saveDetails(StudentLeave $leaveObject)
	{
		...
		if($student_list['leave_category'] == 'Day Outing' || $student_list['leave_category'] == 'Hospital/ Emergency'){
	 ...
	 */
	 ...
	 public function getAcademicSessionDates($organisation_id)
	 {
		$sql = new Sql($this->dbAdapter);
        $select = $sql->select();
        $select->from(array('t1' => 'academic_calendar'))
                    ->columns(array('from_date', 'to_date'))
                ->join(array('t2' => 'academic_calendar_events'), 
                        't1.academic_event = t2.id', array('academic_event', 'academic_session_id'));
        $select->where(array('from_date <= ? ' => date('Y-m-d')));
        $select->where(array('to_date >= ? ' => date('Y-m-d')));
        ...
        foreach($resultSet as $set){
            $academic_session['from_date'] = $set['from_date'];
			$academic_session['to_date'] = $set['to_date'];
	 ...
	 public function getLeaveCategory($leave_category_id)
	 {
		...
		//first get the department, organisation and authtype for the user role
		$select->from(array('t1' => 'student_leave_category'));
		$select->where(array('t1.id' => $leave_category_id));
		...
		$leave_category = NULL;		
		foreach($resultSet as $tmp_data){
			$leave_category = $tmp_data['leave_category'];
		}
		return $leave_category;
	 }
	 ...
	 public function getIndividualAppliedLeaveList($student_id)
	 {
		...
		//officiating role
		$applied_leave_list = array();		
		foreach($resultSet as $tmp_data){
			$applied_leave_list[] = $tmp_data['from_date'];
		}
		return $applied_leave_list;
	 }
		...
		$leave_category = 0;
            ...
            if($tableName == 'student_leave_category'){
            	if($columnName == 'Day Outing'){
					...
					$select->where(array('t1.organisation_id' => $organisation_id)); 
					$select->where(array('t1.leave_category != ?' => 'Weekend Leave'));
					$select->where(array('t1.leave_category != ?' => 'Weekday Leave')); 
            	} else {
            		$select->from(array('t1' => $tableName));
					$select->columns(array('id','leave_category'));
					$select->where(array('t1.organisation_id' => $organisation_id)); 
					$select->where(array('t1.leave_category != ?' => 'Day Outing'));
					$select->where(array('t1.leave_category != ?' => 'Hospital/ Emergency'));  
            	}
            	$stmt = $sql->prepareStatementForSqlObject($select);
	            $result = $stmt->execute();
