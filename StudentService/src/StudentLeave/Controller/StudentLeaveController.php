        ...
        $request = $this->getRequest();
         if ($request->isPost()) {
             ...
             if ($form->isValid()) {
             	$data = $this->getRequest()->getPost('leaveapplication');
             	$from_date = $data['from_date'];
				$leave_category_id = $data['student_leave_category_id']; 

				$leave_category = $this->leaveService->getLeaveCategory($leave_category_id);

             	$appliedLeaveCat = $this->leaveService->getAppliedLeaveCategory($data['student_leave_category_id'], $this->organisation_id); 

				$appliedLeaveList = $this->leaveService->getIndividualAppliedLeaveList($this->student_id); 

             	$check_allocated_hostel = $this->leaveService->checkStudentHostelAllocation($this->student_id); 

				 $applied_date = date('Y-m-d', strtotime($from_date));

				if($applied_date < date('Y-m-d')){ 
					$message = 'Failure';
             		$this->flashMessenger()->addMessage("Sorry! You can't apply for the previous day");
					return $this->redirect()->toRoute('applystdouting');
				}

				if(in_array($applied_date, $appliedLeaveList)){
					$message = 'Failure';
             		$this->flashMessenger()->addMessage("Sorry! you can't apply for the outing since you have applied on ".$applied_date);
					return $this->redirect()->toRoute('applystdouting');
				}
				...
				if(!$check_allocated_hostel && $appliedLeaveCat == 'Weekend Leave'){
					...
					return $this->redirect()->toRoute('applystdouting');
             	...
             	else if($appliedLastDate && $appliedLeaveCat == 'Day Outing'){
             		...
             		if ($pickeddate>=$lastapplieddate) {
             			...
             			$message = 'Failure';
             			$this->flashMessenger()->addMessage("You are not allowed to apply for following reasons
